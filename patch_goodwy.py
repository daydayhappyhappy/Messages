...
def write(p, s):
    ...
    防的是"定位逻辑算错"这一类故障: 脚本里到处是按正则找 span、整段删掉
    (drop_fun / drop_branch / fun_span / _drop_fake_branches ...)。一旦某处
    span 算错 —— 比如找不到配对的 '}' 时一路滑到文件末尾 —— 文件就会被从
    中间切断, 丢掉末尾若干个 '}'。这种损坏有两个特点: 一是编译期才炸, 二是
    报错指向文件末尾, 极难反推是哪一步干的。

    需要说明的是: 这是一道前瞻性防御, 不是修某个已知 bug。正常路径下它完全
    透明, 一次都不会触发; 只有真要写出损坏内容时才会拦。

    规则: 原文件配平 -> 新内容也必须配平, 否则拒绝写入并 warn(宁可漏删)。
          原文件本就不配平 -> 放行(不把已有问题算到这次头上)。
    失败方向是刻意选的: 漏删导致的功能残留, 用户看得见但至少装得上;
    而输出一个编不过的文件, 则是整轮 CI 白跑。
    """
